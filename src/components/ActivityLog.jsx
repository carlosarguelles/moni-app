...
import { ChevronDown, ChevronUp, Plus, Pencil, Trash2, CheckCircle, Clock, History } from 'lucide-react';
      ...
      <div className="space-y-2">
        {displayLogs.map(log => {
          ...
          return (
            ...
            >
              <div className={`shrink-0 w-7 h-7 rounded-full flex items-center justify-center ${config.bg}`}>
                <Icon size={14} className={config.color} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-[13px] text-[var(--color-text)] leading-[1.4]">{log.message}</p>
                <p className="text-[11px] text-[var(--color-text-faint)] mt-1">{formatRelativeTime(log.timestamp)}</p>
              </div>
            </div>
          );
        })}
