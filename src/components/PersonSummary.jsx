...
function Avatar({ name, className = "" }) {
  return (
    <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getAvatarColor(name)} flex items-center justify-center shadow-sm ring-2 ring-white/50 ${className}`}>
            ...
            {people.map(person => {
              ...
              return (
                <div key={person} className="flex items-center gap-4 py-3 border-b border-[var(--stat-border)] last:border-b-0">
                  <Avatar name={person} />
                  <div className="flex-1 min-w-0">
                    <p className="font-bold text-[15px] text-[var(--color-text)] truncate">{person}</p>
                  </div>
