...
export default function App() {
  ...
  function addLog(projectId, log) {
    updateProject(projectId, (p) => ({
      ...
      logs: [
        ...(p.logs || []),
        { id: crypto.randomUUID(), timestamp: new Date().toISOString(), ...log },
      ],
    }));
  }
  ...
  function toggleExpenseStatus(expenseId) {
    ...
    updateProject(activeProjectId, (p) => ({
      ...
      expenses: p.expenses.map((e) =>
        e.id === expenseId ? { ...e, status: e.status === 'paid' ? 'pending' : 'paid' } : e
      ),
    }));
    addLog(activeProjectId, {
      type: 'status_changed',
    ...
    <>
      {projects.length === 0 && !showCreateProject && (
        <div className="text-center py-[60px] px-5 flex flex-col items-center gap-2">
          ...
          <p className="text-[18px] font-bold text-[var(--color-text-dim)]">
            Aún no tienes proyectos.
          </p>
          <p className="text-[14px] text-[var(--color-text-faint)] mb-3">
            Crea tu primer proyecto para empezar a rastrear gastos.
          </p>
          <button
            type="button"
          ...
          {projects.map((project) => {
            ...
            return (
              <button
                ...
                className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-[16px] p-4 cursor-pointer transition-transform active:scale-[0.98] w-full text-left"
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-[16px] text-[var(--color-text)]">
                      {project.name}
                    </h3>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="font-bold text-[16px] text-[var(--color-teal-dark)]">
                      {formatCOP(total)}
                    </span>
                    <button
                      type="button"
      ...
      <header className="fixed top-0 left-0 right-0 z-[100] h-safe-header pt-safe-header px-4 flex items-center backdrop-blur-[14px] bg-[var(--header-bg)] border-b border-[var(--card-border)]">
        {activeProject ? (
          <>
            ...
            <div className="flex-1 min-w-0">
              <h1 className="text-[clamp(16px,4vw,24px)] font-black text-[var(--color-title)] leading-[1.15] whitespace-nowrap overflow-hidden text-ellipsis">
                {activeProject.name}
              </h1>
          ...
          <>
            ...
            <h1 className="text-[20px] ml-[10px] flex-1 font-black text-[var(--color-title)] tracking-[-0.02em] leading-[1.1]">
              Moni
            </h1>
          </>
        )}
        ...
        <div className="max-w-[640px] mx-auto pt-safe-top px-4 pb-safe-fab">
          {activeProject ? (
            <>
              ...
              {activeProject.logs?.length > 0 && <ActivityLog logs={activeProject.logs} />}
            </>
          ) : (
            listContent
          )}
